import { Bell, CheckCheck, ArrowLeftRight, Package, Clock, Info, ShieldAlert, FileText } from "lucide-react";
...
export const NotificationCenter = memo(function NotificationCenter({ notifications, unreadCount, markAsRead, markAllRead }: Props) {
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative h-8 w-8 rounded-xl hover:bg-accent/60 transition-all">
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-destructive text-[9px] font-bold text-destructive-foreground shadow-lg shadow-destructive/30 animate-in zoom-in-50">
              ...
              {notifications.slice(0, 20).map((n) => {
                ...
                return (
                  <div
                    key={n.id}
                    className={cn(
                      "flex items-start gap-3 px-3 py-3 cursor-pointer rounded-xl transition-colors duration-150",
                      "hover:bg-accent/50",
                  ...
                  >
                    ...
                    {!n.lida && (
                      <div className="h-2 w-2 rounded-full bg-primary mt-2 shrink-0 ring-2 ring-primary/20" />
                    )}
                  </div>
                );
              })}
