...
import { ScanLine, Camera, CameraOff, Keyboard, Package, ArrowRight } from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className="p-6 shadow-card space-y-4">
          ...
          <div
            id="barcode-reader"
            className={cn(
              "rounded-lg overflow-hidden border bg-muted/30 transition-all",
              cameraActive ? "h-[240px]" : "h-0 border-0"
            )}
          />
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => handleSearch(code)} className="gradient-primary text-primary-foreground shrink-0">
              Buscar
            </Button>
          </div>
          <p className="text-[11px] text-muted-foreground">
            Conecte um leitor USB, use a câmera ou digite manualmente o código de barras.
          </p>
        </Card>
            ...
            <Card className="p-6 shadow-card space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {result.generico} • {result.concentracao} • {result.forma_farmaceutica}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="rounded-lg bg-muted/30 p-3 text-center">
                  ...
                  <p className="text-xl font-bold">{estoqueTotal}</p>
                </div>
                <div className="rounded-lg bg-muted/30 p-3 text-center">
                  <p className="text-[11px] text-muted-foreground">Est. Mínimo</p>
                  <p className="text-xl font-bold">{result.estoque_minimo}</p>
                </div>
                <div className="rounded-lg bg-muted/30 p-3 text-center">
                  <p className="text-[11px] text-muted-foreground">Lotes Ativos</p>
                  <p className="text-xl font-bold">{lotes.length}</p>
                </div>
                <div className="rounded-lg bg-muted/30 p-3 text-center">
                  <p className="text-[11px] text-muted-foreground">Local</p>
                  <p className="text-lg font-bold font-mono">{result.localizacao || "—"}</p>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {lotes.map((l) => {
                      const daysLeft = Math.ceil(
                        (new Date(l.validade).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
                      );
                      return (
                        <div key={l.id} className="flex items-center gap-3 text-sm rounded-lg border p-2.5">
                          ...
                          <span className="text-muted-foreground ml-auto text-xs">
                            Val: {new Date(l.validade).toLocaleDateString("pt-BR")}
                          </span>
              ...
              <Separator />
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="outline" className="gap-1.5 text-xs" onClick={() => navigate("/entrada")}>
                  <ArrowRight className="h-3 w-3" /> Registrar Entrada
                </Button>
                <Button size="sm" variant="outline" className="gap-1.5 text-xs" onClick={() => navigate("/dispensacao")}>
                  <ArrowRight className="h-3 w-3" /> Dispensar
                </Button>
                <Button size="sm" variant="outline" className="gap-1.5 text-xs" onClick={() => navigate("/etiquetas")}>
                  <ArrowRight className="h-3 w-3" /> Gerar Etiqueta
                </Button>
              </div>
            </Card>
          </motion.div>
